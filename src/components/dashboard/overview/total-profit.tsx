  ...
  React.useEffect(() => {
    const fetchReportData = async () => {
      ...
      try {
        ...
        const response = await axiosInstance.post(`https://fruits-heaven-api.onrender.com/api/v1/order/salesReport`, {
      ...
      <CardContent>
        <Stack direction="row" sx={{ alignItems: 'flex-start', justifyContent: 'space-between' }} spacing={3}>
          <Stack spacing={1}>
            ...
            <Typography variant="h4" className='flex items-center gap-2'>{data?.totalRevenue ? data?.totalRevenue?.toFixed(2) : 0}           <svg 
            id="Layer_1" 
            data-name="Layer 1" 

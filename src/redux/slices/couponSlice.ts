...
const API_URL = `https://fruits-heaven-api.onrender.com/api/v1/coupon`;
...
export const fetchActiveCoupons = createAsyncThunk<
  ...
  void, // Arguments
  { rejectValue: string }
>('coupons/fetchActiveCoupons', async (_, { rejectWithValue }) => {
  try {
...
export const fetchExpiredCoupons = createAsyncThunk<
  ...
  void, // Arguments
  { rejectValue: string }
>('coupons/fetchExpiredCoupons', async (_, { rejectWithValue }) => {
  try {
    const url = new URL(API_URL);
...
// Add a new caCoupon
export const addCoupon = createAsyncThunk<CouponIn, CouponOut, { rejectValue: string }>(
  'coupons/addCoupon',
  async (couponData, { rejectWithValue }) => {
    try {
      const response = await axios.post<CouponOut, AxiosResponse<{ message: string; coupon: CouponIn }, any>>(
        API_URL,
        couponData
      );
      ...
      return rejectWithValue(error.response?.data?.message || error.message || 'Failed to add caCoupon');
    }
  }
);
...
>('coupons/updateCoupon', async ({ id, updatedData }, { rejectWithValue }) => {
  console.log(id);
  try {
    const response = await axios.put<CouponOut, AxiosResponse<{ message: string; coupon: CouponIn }, any>>(
      `${API_URL}/${id}`,
      updatedData
    );
    return response.data.coupon;
    ...
    return rejectWithValue(error.response?.data?.message || error.message || 'Failed to update caCoupon');
  }
});
...
export const updateCouponOrder = createAsyncThunk<
  ...
  { rejectValue: string }
>('products/updateCouponOrder', async ({ id, order }, { rejectWithValue }) => {
  try {
    const response = await axios.patch<CouponOut, AxiosResponse<{ message: string }, any>>(`${API_URL}/${id}`, {
      order: order,
    });
...
>('coupons/deleteCoupon', async (id, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(error.response?.data?.message || error.message || 'Failed to delete caCoupon');
  }
});
...
export const { setRefreshData } = couponsSlice.actions;

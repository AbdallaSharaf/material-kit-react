...
const API_URL = `https://fruits-heaven-api.onrender.com/api/v1/siteSettings/slider`;
...
export const fetchUiSettings = createAsyncThunk<
  ...
  { rejectValue: string }
>('uiSettings/fetchUiSettings', async (params, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(error.response?.data?.message || error.message || 'Failed to fetch uiSettings');
  }
});
...
export const addSetting = createAsyncThunk<
  ...
  { rejectValue: string }
>('uiSettings/addSetting', async (params, { rejectWithValue }) => {
  try {
    ...
    const response = await axios.post<SettingOut, AxiosResponse<{ message: string; setting: SettingIn }, any>>(
      `${API_URL}/${params.key}`,
      { url: imageUrl, redirectUrl: params.settingData.redirectUrl }
    );
    return response.data.setting;
  } catch (error: any) {
    return rejectWithValue(error.response?.data?.message || error.message || 'Failed to add caSetting');
  }
});
...
export const updateSetting = createAsyncThunk<
  ...
  { rejectValue: string }
>('uiSettings/updateSetting', async ({ id, updatedData, key }, { rejectWithValue }) => {
  console.log(id);
  try {
    ...
    const response = await axios.put<SettingOut, AxiosResponse<{ message: string; setting: SettingIn }, any>>(
      `${API_URL}/${key}/${id}`,
      { url: imageUrl }
    );
    return response.data.setting;
    ...
    return rejectWithValue(error.response?.data?.message || error.message || 'Failed to update caSetting');
  }
});
...
export const deleteSetting = createAsyncThunk<
  ...
  { rejectValue: string }
>('uiSettings/deleteSetting', async ({ key, idx }, { rejectWithValue }) => {
  try {
    await axios.delete(`${API_URL}/${key}/${idx}`);
    return { idx, key };
  } catch (error: any) {
    return rejectWithValue(error.response?.data?.message || error.message || 'Failed to delete caSetting');
  }
});
...
export const { setRefreshData } = uiSettingsSlice.actions;
